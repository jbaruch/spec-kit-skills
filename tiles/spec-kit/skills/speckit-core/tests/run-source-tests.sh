...
test_tiles_bash_command_paths() {
    ...
    # Bash commands MUST use .tessl/tiles/tessl-labs/spec-kit/skills/speckit-core/ prefix
    # This is the installed path when users run tessl install
    ((TESTS_RUN++))
    local correct_bash_cmds
    correct_bash_cmds=$(grep -rh "bash .tessl/tiles/tessl-labs/spec-kit/skills/speckit-core/scripts/bash/" "$base"/speckit-*/SKILL.md 2>/dev/null | wc -l)
    if [[ "$correct_bash_cmds" -gt 0 ]]; then
        log_pass "tile bash commands use full .tessl/tiles/... path ($correct_bash_cmds refs)"
    else
        log_fail "tile bash commands don't use .tessl/tiles/tessl-labs/spec-kit/skills/speckit-core/ path"
    fi

    # Bash commands MUST NOT use relative ../speckit-core/ paths (would break at runtime)
    ((TESTS_RUN++))
    local broken_bash_cmds
    broken_bash_cmds=$(grep -rE "^bash \.\./speckit-core/|^   bash \.\./speckit-core/" "$base"/speckit-*/SKILL.md 2>/dev/null | wc -l)
    if [[ "$broken_bash_cmds" -eq 0 ]]; then
        log_pass "no tile bash commands use broken relative ../speckit-core/ path"
    else
        log_fail "found $broken_bash_cmds bash commands with broken relative path"
        grep -rn "bash \.\./speckit-core/" "$base"/speckit-*/SKILL.md 2>/dev/null | head -3
    fi

    # PowerShell commands MUST use .tessl/tiles/ prefix
    ((TESTS_RUN++))
    local correct_pwsh_cmds
    correct_pwsh_cmds=$(grep -rh "pwsh .tessl/tiles/tessl-labs/spec-kit/skills/speckit-core/scripts/powershell/" "$base"/speckit-*/SKILL.md 2>/dev/null | wc -l)
...
test_tiles_markdown_link_paths() {
    log_section "Tiles Markdown Link Paths (must be relative ../speckit-core/)"
    local base="tiles/spec-kit/skills"

    [[ ! -d "$base" ]] && { log_info "tiles/spec-kit/ not found, skipping"; return; }

    # Markdown links to templates SHOULD use relative ../speckit-core/templates/ path
    # This allows tessl tile publish validation to work
    ((TESTS_RUN++))
    local correct_md_links
    correct_md_links=$(grep -rh '\](../speckit-core/templates/' "$base"/speckit-*/SKILL.md 2>/dev/null | wc -l)
    if [[ "$correct_md_links" -gt 0 ]]; then
        log_pass "tile markdown links use relative ../speckit-core/templates/ ($correct_md_links refs)"
    else
        log_fail "tile markdown links don't use ../speckit-core/templates/ path"
    fi

    # Markdown links MUST NOT use .tessl/tiles/ (wrong for validation)
    ((TESTS_RUN++))
    local wrong_md_links
    wrong_md_links=$(grep -rh '\](.tessl/tiles/' "$base"/speckit-*/SKILL.md 2>/dev/null | wc -l)
    ...
    local_md_links=$(grep -rh '\](.claude/skills/' "$base"/speckit-*/SKILL.md 2>/dev/null | wc -l)
