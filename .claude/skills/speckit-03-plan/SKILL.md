...
1. Run setup script (choose based on platform):
   ...
   **Windows (PowerShell):**
   ```powershell
   pwsh .claude/skills/speckit-core/scripts/powershell/setup-plan.ps1 -Json
   ```
...
4. **Agent context update** (choose based on platform):
   ...
   **Unix/macOS/Linux:**
   ```bash
   .claude/skills/speckit-core/scripts/bash/update-agent-context.sh claude
   ```

   **Windows (PowerShell):**
   ```powershell
   pwsh .claude/skills/speckit-core/scripts/powershell/update-agent-context.ps1 -AgentType claude
   ```

   This updates CLAUDE.md with the new technology stack using the [agent-file-template.md](.claude/skills/speckit-core/templates/agent-file-template.md).
